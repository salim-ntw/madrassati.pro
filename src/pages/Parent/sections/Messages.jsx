...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    socket.current.on('connect', () => {
      ...
      socket.current.emit('join', parentId);
    });
  ...
  const loadData = async () => {
    try {
      ...
      const [conversationsRes, contactsRes] = await Promise.all([
        messageAPI.getConversations(parentId),
        messageAPI.getContacts(parentId)
      ]);
  ...
  return (
    <div className='p-4 md:p-6 h-full animate-fadeIn'>
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 md:mb-6 gap-4'>
        <h1 className='text-2xl md:text-3xl font-bold text-gray-800 animate-slideInDown'>
          Messages {child && `· ${child.name}`}
        </h1>
        <button
      ...
      <div className='flex flex-col md:flex-row h-[calc(100vh-180px)] md:h-[calc(100vh-200px)] bg-white rounded-xl shadow-lg overflow-hidden mobile-chat-container'>
        ...
        <div className='w-full md:w-1/3 border-r border-gray-200 flex flex-col mobile-conversation-list md:mobile-conversation-list-none'>
          <div className='p-3 md:p-4 border-b border-gray-200 bg-gray-50'>
            <h2 className='text-base md:text-lg font-semibold text-gray-800'>Teachers</h2>
          ...
          <div className='flex-1 overflow-y-auto hide-scrollbar'>
            {conversations.length === 0 ? (
              <div className='p-4 text-center text-gray-500'>
                ...
                <p className='text-sm'>Start messaging your child's teachers</p>
              </div>
            ) : (
              ...
              >
                <div className='flex items-start justify-between mb-2'>
                  <div className='flex-1 min-w-0'>
                      ...
                      <p className='text-xs md:text-sm text-gray-600 truncate'>Teacher of {conversation.childName}</p>
              ...
              <div className='p-3 md:p-4 border-b border-gray-200 bg-gray-50'>
                <div className='flex items-center justify-between'>
                  <div className='flex-1 min-w-0'>
                    ...
                    <p className='text-xs md:text-sm text-gray-600 truncate'>Teacher of {selectedConversation.childName} ({selectedConversation.childClass})</p>
                  </div>
                ...
                {currentMessages.length === 0 ? (
                  ...
                  currentMessages.map((message) => {
                    ...
                    return (
                      <div
                        key={message._id}
                        className={`flex ${message.senderId?._id?.toString() === parentId ? 'justify-end' : 'justify-start'}`}
                  >
                    <div
                      className={`mobile-message max-w-xs sm:max-w-sm lg:max-w-md px-3 md:px-4 py-2 rounded-lg ${
                            message.senderId?._id?.toString() === parentId
                    ...
                    >
                      ...
                      <p className={`text-xs mt-1 ${
                            message.senderId?._id?.toString() === parentId ? 'text-blue-100' : 'text-gray-500'
                          }`}>
              ...
              >
                ...
                {contacts.map((contact) => (
                  <option key={contact.userId} value={contact.userId}>
                    {contact.name} - {contact.subject} Teacher
                  </option>
                ))}
              </select>
            </div>
